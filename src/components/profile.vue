    ...
    <v-flex xs12>
      <v-container fluid grid-list-md>
        <v-card class="mx-auto" :color="theme.tone" dark>
          <v-card-title>
            <v-icon large left>perm_identity</v-icon>
          ...
          <v-layout justify-center align-center row wrap>
            <v-flex xs10>
              <span>口号(将会显示在做题页面A底端)</span>
            </v-flex>
            <v-flex xs10>
              <v-textarea light solo label="口号" v-model="profile.slogan" auto-grow></v-textarea>
            </v-flex>
            <v-flex xs10>
              <span>QQ号(头像源)</span>
            </v-flex>
            <v-flex xs10>
              <v-text-field light solo label="QQ号" v-model="profile.qq"></v-text-field>
            </v-flex>
          </v-layout>
          <v-card-actions>
            <v-spacer></v-spacer>
          ...
          <v-card-actions>
            <v-list-tile class="grow">
              <v-list-tile-avatar color="grey darken-3">
                <v-img
                  class="elevation-6"
                  :src="`http://q1.qlogo.cn/g?b=qq&nk=${userData.qq}&s=640`"
                ></v-img>
              </v-list-tile-avatar>
...
export default {
  ...
  computed: {
    ...mapState(["userData", "theme", "mainUrl"])
  },
  activated() {
      ...
      this.axios
        ...
        .then(rsp => {
          ...
          if (data.status == 200) {
            ...
            this.$store.state.userData = JSON.parse(
